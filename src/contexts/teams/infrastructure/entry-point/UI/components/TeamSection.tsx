...
const TeamSection: React.FC<TeamSectionProps> = ({ title, teams, loginData }) => {
    const addToFavorites = (teamName: string) => {
                ...
                {teams.length > 0 ? (
                    teams.map((team, index) => (
                        <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
                            <TeamCard team={team} loginData={loginData} addToFavorites={addToFavorites} />
                        </Grid>
                    ))
                ) : (
